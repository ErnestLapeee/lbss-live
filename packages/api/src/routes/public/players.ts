    ...
    try {
      ...
      if (await gamesHavePlayoffSeriesId()) {
        ...
        playoffRows = await db.select({
          ...
          buntSingles: sql<number>`COALESCE(SUM(${playerGameBatting.buntSingles}),0)`.as('bunt_singles'),
          strikeoutsLooking: sql<number>`COALESCE(SUM(${playerGameBatting.strikeoutsLooking}),0)`.as('strikeouts_looking'),
          strikeoutsSwinging: sql<number>`COALESCE(SUM(${playerGameBatting.strikeoutsSwinging}),0)`.as('strikeouts_swinging'),
          pickedOff: sql<number>`COALESCE(SUM(${playerGameBatting.pickedOff}),0)`.as('picked_off'),
          fieldersChoice: sql<number>`COALESCE(SUM(${playerGameBatting.fieldersChoice}),0)`.as('fielders_choice'),
          catcherInterference: sql<number>`COALESCE(SUM(${playerGameBatting.catcherInterference}),0)`.as('catcher_interference'),
          groundedIntoTriplePlay: sql<number>`COALESCE(SUM(${playerGameBatting.groundedIntoTriplePlay}),0)`.as('grounded_into_triple_play'),
  ...
  app.get<{ Params: { slug: string }; Querystring: { seasonId?: string } }>('/:slug/pitching-stats', async (request, reply) => {
    try {
      ...
      if (await gamesHavePlayoffSeriesId()) playoffRows = await db.select({
        ...
        holds: sql<number>`COALESCE(SUM(${playerGamePitching.holds}), 0)`.as('holds'),
        saveOpportunities: sql<number>`COALESCE(SUM(${playerGamePitching.saveOpportunities}), 0)`.as('save_opportunities'),
        blownSaves: sql<number>`COALESCE(SUM(${playerGamePitching.blownSaves}), 0)`.as('blown_saves'),
        inheritedRunners: sql<number>`COALESCE(SUM(${playerGamePitching.inheritedRunners}), 0)`.as('inherited_runners'),
        inheritedRunnersScored: sql<number>`COALESCE(SUM(${playerGamePitching.inheritedRunnersScored}), 0)`.as('inherited_runners_scored'),
        strikeoutsLooking: sql<number>`COALESCE(SUM(${playerGamePitching.strikeoutsLooking}), 0)`.as('strikeouts_looking'),
        strikeoutsSwinging: sql<number>`COALESCE(SUM(${playerGamePitching.strikeoutsSwinging}), 0)`.as('strikeouts_swinging'),
        balls: sql<number>`COALESCE(SUM(${playerGamePitching.balls}), 0)`.as('balls'),
        strikes: sql<number>`COALESCE(SUM(${playerGamePitching.strikes}), 0)`.as('strikes'),
        firstPitchStrikes: sql<number>`COALESCE(SUM(${playerGamePitching.firstPitchStrikes}), 0)`.as('first_pitch_strikes'),
        firstPitchTotal: sql<number>`COALESCE(SUM(${playerGamePitching.firstPitchTotal}), 0)`.as('first_pitch_total'),
        teamName: teams.name,
        seasonYear: seasons.year,
      })
